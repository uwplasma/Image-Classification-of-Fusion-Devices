...
import os # need this to save into directory
...
def plot_boundary(self, r=0.1, ntheta=80, nphi=150, ntheta_fourier=20, nsections=8,
    ...
    # ---------------------------------I COMMENTED THIS TO GET RID OF THE POLODIAL PLOT--------------------------------
    ...
    if fieldlines==False:
        if colormap==None:
            ...
            cmap = cm.viridis   #Change to Gray colormap

            # Add a light source so the surface looks brighter
            ls = LightSource(azdeg=0, altdeg=10)
            cmap_plot = ls.shade(Bmag, cmap, norm=norm, blend_mode='soft')
        ...
        # fig = plt.figure(constrained_layout=False, figsize=(4.5, 8))    --> I COMMENTED THIS OUT
        ...
        # --------------------------------------------------------------------------------------    
            # THIS CODE PLOTS THE SUBPLOTS AS IT SELF
            fig = plt.figure(figsize=(6,6), dpi=180)  # You can adjust the figsize as needed

            # Create a new 3D subplot in each figure
            ax = fig.add_subplot(111, projection='3d')

            # Plot the 3D data using the create_subplot function (or directly if you know how to do it manually)
            create_subplot(ax, x_2D_plot, y_2D_plot, z_2D_plot, cmap_plot, elev=elevParams[i], azim=azim_default, **kwargs)

            #saving image to directory
            file_name = f'{savefig}_{i+1}.png'
            save_dir = './stel_images'
